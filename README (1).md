...
**Nama Tim:** _(isi nama tim kamu)_

| Nama Anggota | Job Desk / Pembagian Tugas |
| :--- | :--- |
| **[NAMA ANGGOTA 1]** | **Frontend & Axios Specialist:**<br>• Merancang seluruh tampilan antarmuka (UI/UX) aplikasi agar responsif dan intuitif.<br>• Bertanggung jawab penuh atas integrasi library **Axios** untuk komunikasi dengan GitHub REST API.<br>• Mengimplementasikan fitur pencarian repository (`HomeScreen.js`) dan halaman detail repository (`DetailScreen.js`). |
| **[NAMA ANGGOTA 2]** | **Backend, State & Firebase Specialist:**<br>• Mengatur logika state aplikasi dan penyimpanan data lokal di perangkat menggunakan **AsyncStorage**.<br>• Bertanggung jawab penuh atas seluruh integrasi layanan **Firebase** (Authentication & Cloud Firestore).<br>• Mengimplementasikan sistem Login/Register, penyimpanan favorit dengan catatan pribadi (CRUD Firestore), dan manajemen sesi pengguna via `AuthContext.js`. |

---

## 🛠️ Fitur Utama Aplikasi

1. **Cari & Jelajahi Repository Robotika:** Mencari repository open-source bertema robotika dari GitHub secara real-time menggunakan **Axios** ke GitHub Search API, menampilkan nama, deskripsi, jumlah ⭐ star, dan bahasa pemrograman.

2. **Login & Register Akun:** Sistem autentikasi pengguna menggunakan **Firebase Authentication** (Email/Password), dengan manajemen sesi otomatis via `onAuthStateChanged` — navigasi berpindah otomatis tanpa kode manual saat status login berubah.

3. **Simpan Favorit & Catatan Pribadi:** Pengguna dapat menyimpan repository pilihan beserta catatan pribadi ke **Cloud Firestore** (CRUD), data tampil secara realtime menggunakan `onSnapshot` dan dapat dihapus kapan saja.
...
2. Install dependencies:
   ...
   npm install
   ```
3. Buat project di [Firebase Console](https://console.firebase.google.com), aktifkan **Authentication (Email/Password)** dan **Firestore Database**, lalu isi kredensial di `firebaseConfig.js`
4. Jalankan project:
   ```bash
   npx expo start
   ```
5. Scan QR code menggunakan aplikasi **Expo Go** di HP
